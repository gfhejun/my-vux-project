<template>
	...
	<group title="接待单位">
		...
		<x-input class="text-right" title="陪同员工" placeholder="点击选择员工" :readonly="true" :value.sync="employee" text-align="right" v-on:click="openSelectUserPop('选择陪同员工')"></x-input>
	...
	<group v-if="$route.type=='new'">
		<x-input title="审批人" :value.sync="approver" placeholder="点击选择审批人" :readonly="true" class="text-right" text-align="right" v-on:click="openSelectUserPop('选择审批人')"></x-input>
	</group>
	...
	<select-user-popup :title="popTitle" :pop-show="popShow"></select-user-popup>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			openSelectUserPop: function (title) {
				this.popShow = true;
				this.popTitle = title;
			}
		},
